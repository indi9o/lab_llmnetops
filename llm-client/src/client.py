...
async def run_chat_loop():
    ...
    print("DEBUG: Opening SSE client...", flush=True)
    async with sse_client(MCP_SERVER_URL) as (read_stream, write_stream):
        print("DEBUG: SSE client opened, creating session...", flush=True)
        async with ClientSession(read_stream, write_stream) as session:
            print("DEBUG: Session created, initializing...", flush=True)
            await session.initialize()
            print("DEBUG: Session initialized!", flush=True)
            
            # List available tools
            tools = await session.list_tools()
            print(f"Connected! Available tools: {[t.name for t in tools.tools]}", flush=True)
...
Do NOT make up information - always use the tools to get real data."""
            ...
            while True:
                try:
                    ...
                    print(f"DEBUG: Got input: {user_input}", flush=True)
                    if user_input.lower() in ['quit', 'exit']:
                        break
                    
                    messages.append({'role': 'user', 'content': user_input})
                    print("DEBUG: Added to messages, calling Ollama...", flush=True)
                    
                    print(f"DEBUG: Sending request to Ollama (model={MODEL_NAME})...", flush=True)
                    # Call Ollama
                    ...
                    print("DEBUG: Received response from Ollama.", flush=True)
                    
                    # Check for tool calls
                    message = response['message']
                    print(f"DEBUG: Message content: {message.get('content')}")
                    print(f"DEBUG: Tool calls: {message.get('tool_calls')}")
                    ...
                    if message.get('tool_calls'):
                        ...
                        print("DEBUG: Sending tool results back to Ollama...")
